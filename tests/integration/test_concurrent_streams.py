...
def test_stream_isolation_one_failure_doesnt_affect_others(mocker, mock_concurrent_cameras):
    ...
    for call in info_calls:
        if (len(call[0]) >= 2 and
            call[0][0] == 'Stream for Camera %d started successfully.' and
            isinstance(call[0][1], int)):
...
def test_frontend_stream_naming_pattern(client):
    ...
    assert 'for (let i = 1; i <= totalCameras; i++)' in response.text or \
           'for(let i=1;i<=totalCameras;i++)' in response.text or \
           'for (let i = 1; i <= totalCameras; i++)' in response.text.replace(' ', '')
